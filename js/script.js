...
var STROKEWIDTH = 2;
var AXISWIDTH = 1;
var BRANCH_CUT_THRESHHOLD = 100;
//
var FRAMESIZE = Math.round(Math.min(window.innerHeight*(0.8),window.innerWidth*(0.7)));
//
...
function resetRange()
{
	document.getElementById("ZMAXX").value = 10;
	document.getElementById("ZMINX").value = -10;
	document.getElementById("ZMAXY").value = 10;
	document.getElementById("ZMINY").value = -10;
	document.getElementById("WMAXX").value = 10;
	document.getElementById("WMINX").value = -10;
	document.getElementById("WMAXY").value = 10;
	document.getElementById("WMINY").value = -10;
	Z_MAX_X = 10;
	Z_MIN_X = -10;
	Z_MAX_Y = 10;
	Z_MIN_Y = -10;
...
function zMap()
{
	...
	zContext.stroke();
	//
	var zplane = zContext.getImageData(0,0,zCanvas.zidth,zCanvas.height);
	var data = zplane.data;
	var prevx = -1;
	var prevy = -1;
	...
	for(var i = 0; i < clickDrag.length; i++)
	{
		...
		var out_x = Math.round(((out_re - Z_MIN_X)/(Z_MAX_X - Z_MIN_X))*zCanvas.width)
		var out_y = Math.round((1-((out_im - Z_MIN_Y)/(Z_MAX_Y - Z_MIN_Y)))*zCanvas.height)
		...
		if(i != 0)
		{
			...
			{
				if((distance(out_x,out_y,prevx,prevy)/distance(clickX[i-1],clickY[i-1],clickX[i],clickY[i]))<BRANCH_CUT_THRESHHOLD)
					zContext.moveTo(prevx, prevy);
				else
					zContext.moveTo(out_x, out_y);
			}
			else
			{
				zContext.moveTo(out_x, out_y);
			}
			zContext.lineTo(out_x,out_y);
			zContext.closePath();
			zContext.stroke();
		}
		prevx = out_x;
